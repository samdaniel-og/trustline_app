...
function ChatH() {
  ...
  useEffect(() => {
    ...
    // Check if there's a chat to open from the complaint page
    const chatComplaint = JSON.parse(localStorage.getItem("currentChatComplaint"));
    if (chatComplaint) {
      // Find or create chat for this complaint
      let chat = storedChats.find(c => c.complaintId === chatComplaint.id);
      
      if (!chat) {
        chat = {
          id: Date.now(),
          complaintId: chatComplaint.id,
          clientName: chatComplaint.userName,
          clientCode: chatComplaint.userCode,
          category: chatComplaint.category,
          lastMessage: "Chat started",
          lastMessageTime: new Date().toISOString()
        };
        
        const updatedChats = [...storedChats, chat];
      ...
      setActiveChat(chat);
      localStorage.removeItem("currentChatComplaint");
    }
  }, []);
      ...
      <nav className="navbar navbar-expand-lg navbar-dark bg-custom px-4">
        ...
        <div className="collapse navbar-collapse" id="navbarNav">
          <ul className="navbar-nav me-auto ms-3">
            ...
            <li className="nav-item"><Link className="nav-link" to="/complaintH">Complaint</Link></li>
            <li className="nav-item"><Link className="nav-link active" to="/chatH">Chat</Link></li>
            ...
            <div className="chat-list" style={{ maxHeight: 'calc(100vh - 200px)', overflowY: 'auto' }}>
              {chats.length === 0 ? (
                <div className="text-center p-5">
                  ...
                  <small>Start a chat from the complaint page</small>
                </div>
              ) : (
                chats.map(chat => (
                  <div 
                  ...
                  >
                    <div className="d-flex justify-content-between align-items-start">
                      <div className="flex-grow-1">
                        <h6 className="mb-1">{chat.clientName} ({chat.clientCode})</h6>
                        <p className="mb-1 text-muted small">{chat.category}</p>
            ...
            {activeChat ? (
              <div className="d-flex flex-column h-100">
                ...
                <div className="border-bottom p-3 bg-light">
                  <div className="d-flex justify-content-between align-items-center">
                    <div>
                      <h5 className="mb-0">{activeChat.clientName} ({activeChat.clientCode})</h5>
                      <small className="text-muted">{activeChat.category}</small>
                    </div>
                    <div>
                      <Link 
                        to="/complaintH" 
                ...
                >
                  {messages[activeChat.id]?.length === 0 ? (
                    ...
                    messages[activeChat.id]?.map(message => (
                      ...
                      >
                        <div 
                          className={`p-3 rounded ${message.sender === currentUser.username ? 'bg-primary text-white' : 'bg-light'}`}
                          style={{ maxWidth: '70%' }}
                        >
                          {message.type === "media" ? (
                            <div>
                          ...
                          <small className={`d-block text-end ${message.sender === currentUser.username ? 'text-white-50' : 'text-muted'}`}>
              ...
              <div className="d-flex justify-content-center align-items-center h-100">
                <div className="text-center">
                  ...
                  <p className="text-muted">Select a chat from the list or start a new conversation from the complaint page</p>
                </div>
              </div>
            )}
          </div>
